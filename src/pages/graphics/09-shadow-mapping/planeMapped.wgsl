...
@group(2) @binding(0) var shadow_sampler : sampler;
...
fn main_vs(@location(0) position : vec4f, @location(1) uv : vec2f) -> VSOut
{
    ...
    vsOut.shadow_lookup = vec3f(pos_from_light.xy * vec2(0.25, -0.25) + vec2(0.5, 0.25), pos_from_light.z);
...
fn main_fs(@location(0) uv : vec2f, @location(1) shadow_lookup : vec3f) -> @location(0) vec4f
{
    ...
    var visibility = textureSample(shadow_map, shadow_sampler, uv);
